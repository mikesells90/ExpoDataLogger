...
from analytics import render_war_room
...
from utils import from_json, make_uuid, now_iso, to_json
...
st.caption("Fast capture + real-time competitive intelligence for trade show field ops.")
...
with left_col:
    ...
    with st.form("capture_form", clear_on_submit=False):
        ...
        if quick_add_mode:
            # Fast-path capture for overwhelmed show-floor moments.
...
if submitted:
    if not brand_name.strip():
    ...
    elif not st.session_state.get("quick_add_mode", False) and len(form_data.get("flavor_mode", [])) > 2:
        ...
        payload.update(form_data if not st.session_state.get("quick_add_mode", False) else {})
        ...
        try:
            ...
            # Recompute clusters after each insert for real-time war-room updates.
            update_clusters(session)
            saved = session.query(BoothEntry).filter(BoothEntry.id == entry.id).first()

            st.success("Intel captured.")
            k1, k2, k3, k4 = st.columns(4)
            k1.metric("Archetype", saved.archetype or "Unclassified")
            k2.metric("Blue Ocean Score", f"{saved.blue_ocean_score:.1f}")
            k3.metric("Threat Score", f"{saved.threat_score:.1f}")
            k4.metric("Cluster Label", saved.cluster_label or "Pending")
...
with right_col:
    session = SessionLocal()
    try:
        df = get_entries_df(session)
        if not df.empty:
            df["flavor_mode"] = df["flavor_mode"].apply(from_json)
            df["ingredient_signals"] = df["ingredient_signals"].apply(from_json)
            df["traffic_behavior"] = df["traffic_behavior"].apply(from_json)
            df["objections"] = df["objections"].apply(from_json)
        render_war_room(df)
    finally:
        session.close()
